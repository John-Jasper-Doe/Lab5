...
void model::add_element(std::unique_ptr<element_t<model_type_t> >&& elem) {
  elements_.push_back(std::move(elem));
  notify_update();
}

void model::delete_element(const point_t<model_type_t>& point) {
...
const std::list<std::unique_ptr<element_t<model_type_t> > >& model::elements() const {
  return elements_;
}
